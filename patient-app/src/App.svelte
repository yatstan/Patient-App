<script lang="ts">
  ...
  async function startRecording() {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        await sendToSTTBackend(resampledBlob);
      };
  ...
  async function resampleAudioTo16kHz(audioBlob: Blob): Promise<Blob> {
    ...
    return bufferToWaveBlob(renderedBuffer, 16000);
  }

  function bufferToWaveBlob(buffer: AudioBuffer, sampleRate: number): Blob {
  ...
  const sendToSTTBackend = async (audioBlob: Blob) => {
    ...
    try {
      const response = await fetch('https://3000-yatstan-patientapp-5soy7udcitp.ws-us116.gitpod.io/stt', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleTTSResponse = async (text: string) => {
    try {
      const response = await fetch('https://3000-yatstan-patientapp-5soy7udcitp.ws-us116.gitpod.io/tts', {
        method: 'POST',
